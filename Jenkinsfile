    ...
    stages {
       ...
       stage('Deploy MySQL to DEV') {
            steps {
                ...
                sh "docker run --name mysql-db --rm --network dev -v khalid-mysql-data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=${mysql-db} -e MYSQL_DATABASE=db_example  -d mysql:8.0 "
                sh 'sleep 20'
                sh "docker exec -i mysql-db mysql --user=root --password=${mysql-db} < backup.sql"
            }
        }
