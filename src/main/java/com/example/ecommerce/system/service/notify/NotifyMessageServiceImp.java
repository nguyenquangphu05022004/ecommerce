...
import static com.example.ecommerce.system.enums.SysErrorCodeConstants.UPDATE_NOTIFY_MESSAGE_DENIED;
...
public class NotifyMessageServiceImp implements NotifyMessageService{
    ...
    @Override
    public PageResult<NotifyMessage> getNotifyMessagePage(NotifyMessagePageReqVO reqVO) {
        Page<NotifyMessage> pageNotifyMessage =
                this.notifyMessageRepository.findAllByUserMemberIdAndNotifyTemplateId(
                        reqVO.getUserId(), reqVO.getNotifyTemplateId(),
                        PageRequest.of(reqVO.getPage() - 1, reqVO.getLimit())
                );
    ...
    public NotifyMessage getNotifyMessageById(Long id) {
        return this.notifyMessageRepository.findById(id)
    ...
    @Override
    public void updateReadNotifyMessage(Long notifyMessageId, Long userId) {
        NotifyMessage notifyMessage = getNotifyMessageById(notifyMessageId);
        if(notifyMessage.getUserMember().getId().equals(userId)) {
            notifyMessage.markAsRead();
            this.notifyMessageRepository.save(notifyMessage);
            return;
        }
        throw exception(UPDATE_NOTIFY_MESSAGE_DENIED);
    }
